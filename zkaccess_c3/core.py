...
class C3:
    ...
    def set_user(
        ...
        went from 29 to 30 users, CardNo readable back via GETDATA):
          ...
          cmd     0x07
          header  = bytes([1, 7, 1, 2, 3, 4, 5, 6, 7])  (table=1, 7 fields, idx 1..7)
          record  = <size><value> per field, little-endian, minimal byte length
                    (0 encodes as a single 0x00 byte), in field order:
                    CardNo, Pin, Password, Group, StartTime, EndTime, SuperAuthorize

        NOTE: password is currently always sent empty (b"\\x00") - only that
        shape was validated. Deleting/editing a user is NOT validated yet
        (see delete_user below); do not assume this is symmetrical with reads.

        Returns the CardNo written (check with get_device_data("user")).
        """
        def iv(v: int) -> bytes:
            if v == 0:
                return b"\x00"
        ...
        card_b = iv(card)
        pin_b = iv(pin if pin is not None else card // 1000)
        record = (
            bytes([len(card_b)]) + card_b
            + bytes([len(pin_b)]) + pin_b
            + b"\x00"  # password: only the empty case was validated
            + bytes([len(iv(group_id))]) + iv(group_id)
            + bytes([len(iv(start_time))]) + iv(start_time)
            + bytes([len(iv(end_time))]) + iv(end_time)
            + bytes([len(iv(super_authorize))]) + iv(super_authorize)
        )
        header = bytes([1, 7, 1, 2, 3, 4, 5, 6, 7])
